...
import {
  PaymentSortableService,
  SortEvent,
} from "../payment-sortable.directive";
...
export class PaymentComponent implements OnInit {
  ...
  @ViewChildren(PaymentSortableService)
  headers: QueryList<PaymentSortableService>;
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [
      { label: "Tables" },
      { label: "Payment table", active: true },
    ];
  ...
  loadMetricsOptimized(): void {
    this.service.getPayments().subscribe((payments) => {
      ...
      this.metrics = {
        ...
        pendingCount: payments.filter((p) => p.paymentStatus === "Pending")
          .length,
        overdueCount: payments.filter((p) => p.paymentStatus === "Overdue")
          .length,
  ...
  private _fetch(){
    this.categoryService.getOfferCategoryById({id:'jjjjj'})
  }
}
